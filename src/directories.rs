...
impl Directories {
    ...
    pub fn for_provider(base_dir: impl AsRef<Path>, provider: impl AsRef<Path>) -> Self {
        Self::new(base_dir.as_ref().join(provider.as_ref()))
    }
    ...
    pub fn for_emoji(&self, size: u32, emoji: &EmojiImage) -> PathBuf {
        self.base_dir.join(format!("{size}/{}.png", emoji.unified))
    }
}
